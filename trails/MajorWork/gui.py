...
class ParkingApp:
    ...
    def process_frame(self, frame):
        ...
        # ONNX Model Inference (Cars)
        if self.model_onnx is not None:
            blob = cv2.dnn.blobFromImage(frame, 1/255, (self.onnx_img_w, self.onnx_img_h), swapRB=True, mean=(0, 0, 0), crop=False)
            ...
            x_scale = frame_w / self.onnx_img_w
            y_scale = frame_h / self.onnx_img_h
        ...
        # YOLOv8n Inference (Cars)
        if self.model_yolov8 is not None:
            results_v8 = self.model_yolov8(frame, verbose=False, conf=conf_thresh, imgsz=imgsz)
        ...
        if len(all_car_boxes) > 0:
            indices = cv2.dnn.NMSBoxes(all_car_boxes, all_car_scores, conf_thresh, 0.45)
